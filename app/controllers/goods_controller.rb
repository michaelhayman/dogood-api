class GoodsController < ApiController
  ...
  def resource_params
    params.require(:good).permit(:caption, :evidence, :user_id, :category_id, :lat, :lng, :location_name, :location_image, :done, nominee_attributes: [ :full_name, :email, :phone, :user_id, :twitter_id, :facebook_id, :avatar ], entities_attributes: [ :entityable_id, :entityable_type, :link, :link_id, :link_type, :title, range: [] ])
  end
